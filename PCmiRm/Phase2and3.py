"""
Algorithm to select and compare two groups of cooperative modules of differentially expressed miRNAs and its targets (found by database retrieveing)

Inputs: Only .csv or .tsv files in a separated folder numerated necessarily in the order below as "File1" for item number 1 and so on...
...
def readinput(): #read all and access the content of the tables transforming each in a list of modules and then deletes the blank spaces
    inputs = glob('./data/inputs/secondphaseinputs/File*.csv') + glob('./data/inputs/secondphaseinputs/File*.tsv') + glob('./data/inputs/secondphaseinputs/File*.txt')
...
def main():
    ...
    minm = int(input("What is the minimum number of differentially expressed (DE) miRNAs accepted in the modules? ")) #for the user to choose the min DE miRNAs
    minTSG = int(input("What is the minimum number of tissue-specific (TS) genes accepted in the modules? "))
    minpescore = int(input("What is the percentage cutoff of the highest scores you want to use? (enter '100' to select all scores)"))
    if minm + minTSG < 1:
